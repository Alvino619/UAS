...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <div class="px-4 py-5 sm:px-6 border-b border-gray-200">
            ...
            <p class="mt-1 text-sm text-gray-500">Fill in the details to create a new course.</p>
        </div>
        
        <form action="{{ route('courses.manage.store') }}" method="POST" enctype="multipart/form-data" class="px-4 py-5 sm:p-6">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea id="content" name="content" rows="6" required
                        class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">{{ old('content') }}</textarea>
                    @error('content')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
